...
## SH / GH / 민간 — 통합 소스로 한 번에

SH·GH는 단독 OpenAPI가 약하고(주택유형·공급호수 누락), 민간은 단독 API가 없다.
대신 **국토교통부 마이홈포털 공공주택 모집공고 조회 서비스**(`data.go.kr/data/15108420`)가
**공공임대 + 공공지원 민간임대를 통합** 제공한다 → SH/GH/민간을 어댑터 하나로 커버.

- 어댑터: `myhome_source.py` (LH는 전용 API가 더 풍부하므로 제외)
- 활성화: `15108420` 활용신청(자동승인, 기존 키 사용) → 참고문서로 `ENDPOINT`/필드 매핑 확정
  → `config.json`의 `myhome.enabled = true`

- [x] LH (공고+공급정보+상세정보)
- [ ] 마이홈 통합(SH/GH/민간) — `myhome_source.py` 필드 매핑 완료 후 활성화
